...
export class AtendimentoService implements IService<Atendimento> {
  ...
  getAgenda(page: number, size: number, termoBusca?:string): Observable<any[]> {
    termoBusca = termoBusca ? termoBusca : "";
    let url = this.apiUrl + 'agenda?page=' + page + '&size=' + size + "&termoBusca=" + termoBusca;
    return this.http.get<any>(url);
  }
  
  getEmAndamento(page: number, size: number, termoBusca?:string): Observable<any[]> {
    termoBusca = termoBusca ? termoBusca : "";
    let url = this.apiUrl + 'emAndamento?page=' + page + '&size=' + size + "&termoBusca=" + termoBusca;
    return this.http.get<any>(url);
  }

  getFinalizados(page: number, size: number, termoBusca?:string): Observable<any[]> {
    termoBusca = termoBusca ? termoBusca : "";
    let url = this.apiUrl + 'finalizados?page=' + page + '&size=' + size + "&termoBusca=" + termoBusca;
    return this.http.get<any>(url);
  }

  getCancelados(page: number, size: number, termoBusca?:string, apiUrl?: string): Observable<any[]> {
    termoBusca = termoBusca ? termoBusca : "";
    let url = this.apiUrl + 'cancelados?page=' + page + '&size=' + size + "&termoBusca=" + termoBusca;    
    return this.http.get<any>(url);
  }
